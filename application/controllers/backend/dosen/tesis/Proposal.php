...
class Proposal extends CI_Controller {
    ...
    public function penguji() {
        ...
        $data = array(
            ...
            'tesis' => $this->tesis->read_permintaan_penguji_prodi($this->session_data['username'], UJIAN_TESIS_PROPOSAL, $id, 'pengajuan')
        );
        $this->load->view('backend/index_sidebar', $data);
    }
    ...
    public function riwayat() {
        ...
        $data = array(
            ...
            'title' => 'Tesis - Penguji Proposal',
            'subtitle' => 'Data',
            'section' => 'backend/dosen/tesis/proposal/riwayat',
            // DATA //
            'max_id_prodi' => $this->tesis->read_max_prodi_s2(),
            'prodi' => $this->tesis->read_prodi_s2(),
            //'tesis' => $this->tesis->read_permintaan_penguji($this->session_data['username'], UJIAN_TESIS_PROPOSAL)
            'tesis' => $this->tesis->read_permintaan_penguji_prodi($this->session_data['username'], UJIAN_TESIS_PROPOSAL, $id, 'riwayat')
        );
        $this->load->view('backend/index_sidebar', $data);
    }
        ...
        if ($hand == 'center19') {
            ...
            if (!empty($ujian)) { // JIKA SUDAH ADA
                ...
                if ($cek_jadwal_bentrok['status']) {
                    $penguji = $this->tesis->read_penguji($id_ujian);
                    if (!empty($penguji)) {
                        $cek_penguji_bentrok = $this->penjadwalan->cekBentrokPengujis($penguji, $data['tanggal'], $data['id_jam']);
                        if ($cek_penguji_bentrok['status']) {
                            $bentrok_pembimbing_satu = $this->penjadwalan->cekBentrokPenguji($tesis->nip_pembimbing_satu, $data['tanggal'], $data['id_jam']);
                            if ($bentrok_pembimbing_satu['status']) {
                                $bentrok_pembimbing_dua = $this->penjadwalan->cekBentrokPenguji($tesis->nip_pembimbing_dua, $data['tanggal'], $data['id_jam']);
