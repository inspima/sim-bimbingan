...
class Disertasi_terbuka extends CI_Controller {
	...
	public function cetak_sk_ujian()
	{
		...
		if ($hand == 'center19') {
			...
			$kode_dokumen = $this->dokumen->generate_kode(DOKUMEN_SK_UJIAN_DISERTASI, DOKUMEN_JENIS_DISERTASI_UJIAN_TERBUKA_STR, $disertasi->nim, $jadwal->tanggal);
			...
			// QR
			$qr_image_dokumen_name = $this->qrcode->generateQrImageName('Dokumen SK Ujian', 'Terbuka', $disertasi->nim, $jadwal->tanggal);
			...
			$data_dokumen = [
				...
				'nama' => 'SK Ujian Terbuka - ' . $disertasi->nama,
			...
			$page = 'backend/prodi/doktoral/terbuka/cetak_sk_ujian';
			$size = 'legal';
			$this->pdf->setPaper($size, 'potrait');
			$this->pdf->filename = "SURAT KEPUTUSAN - UJIAN TERBUKA - " . $disertasi->nim . '.pdf';
			$this->pdf->load_view($page, $data);
		} else {
			$this->session->set_flashdata('msg-title', 'alert-danger');
			$this->session->set_flashdata('msg', 'Terjadi Kesalahan');
