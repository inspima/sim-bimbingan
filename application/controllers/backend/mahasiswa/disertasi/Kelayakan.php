...
class Kelayakan extends CI_Controller {
    ...
    public function index() {
        $data = array(
            ...
            'section' => 'backend/mahasiswa/disertasi/kelayakan/index',
            // DATA //
            //'mahasiswa'      => $this->mahasiswa->read_aktif($this->session_data['username']),
            'disertasi' => $this->disertasi->read_kelayakan_mahasiswa($this->session_data['username'])
        );
    ...
    public function info() {
        ...
        $data = array(
            'title' => 'Modul (Mahasiswa)',
            'subtitle' => 'Disertasi - Ujian Kelayakan',
            'section' => 'backend/mahasiswa/disertasi/kelayakan/info',
            'use_back' => true,
            'back_link' => 'mahasiswa/disertasi/kelayakan',
            // DATA //
    ...
    public function add() {
        ...
        if ($read_aktif) {
            ...
            redirect('mahasiswa/disertasi/kelayakan');
        } else {
            $data = array(
                // PAGE //
                'title' => 'Modul (Mahasiswa)',
                'subtitle' => 'Pengajuan Ujian Kelayakan',
                'section' => 'backend/mahasiswa/disertasi/kelayakan/add',
                'use_back' => true,
                'back_link' => 'mahasiswa/disertasi/kelayakan',
                'mdosen' => $this->dosen->read_aktif_alldep(),
                // DATA //
    ...
    public function save() {
        ...
        if ($hand == 'center19') {
            ...
            if (!$this->upload->do_upload('berkas_kelayakan')) {
                ...
                $this->session->set_flashdata('msg', 'Anda telah melakukan pengajuan kelayakan..');
                redirect('mahasiswa/disertasi/kelayakan');
            }
        } else {
            $this->session->set_flashdata('msg-title', 'alert-danger');
    ...
    public function edit() {
        ...
        $data = array(
            ...
            'title' => 'Modul (Mahasiswa)',
            'subtitle' => 'Pengajuan Ujian Kelayakan',
            'section' => 'backend/mahasiswa/modul/kelayakan_edit',
            // DATA //
            'departemen' => $this->departemen->read(),
            'gelombang' => $this->gelombang->read_berjalan(),
            'kelayakan' => $this->disertasi->detail($id, $username)
        );
        ...
        if ($data['kelayakan']) {
            ...
            $data['linkback'] = 'dashboardm/modul/kelayakan';
            $this->load->view('backend/index_sidebar', $data);
        }
    }
    ...
    public function update() {
        ...
        if ($hand == 'center19') {
            ...
            if ($judul == $read_judul->judul) {
                ...
                redirect('dashboardm/modul/kelayakan');
            } else {
                $data = array(
                    'id_departemen' => $this->input->post('id_departemen', TRUE),
                );
    ...
    public function promotor_save() {
        ...
        if ($hand == 'center19') {
            ...
            if ($cek_promotor) {
                ...
                redirect('mahasiswa/disertasi/kelayakan/add/' . $id_disertasi);
            } else {
