...
class Mkpt extends CI_Controller {
    ...
    public function bimbingan() {
        ...
        $data = array(
            ...
            'section' => 'backend/mahasiswa/tesis/mkpt/bimbingan',
            'use_back' => true,
            'back_link' => 'mahasiswa/tesis/mkpt',
            // DATA //            
    ...
    public function bimbingan_save() {
        ...
        if ($hand == 'center19') {
            ...
            $config['upload_path'] = './assets/upload/mahasiswa/tesis/mkpt/bimbingan';
            $config['allowed_types'] = 'pdf';
            ...
            if($_FILES['berkas_bimbingan_mkpt']['size'] != 0){
                if ($this->upload->do_upload('berkas_bimbingan_mkpt')) {
                    $data = array(
                        'id_tesis' => $id_tesis,
                        'jenis' => UJIAN_TESIS_MKPT,
                        'status' => 1,
                        'tanggal' => $tgl,
                        'hal' => $hal,
                        'file' => $file_name,
            ...
            redirect('mahasiswa/tesis/mkpt/bimbingan/'.$id_tesis);
        } else {
            $this->session->set_flashdata('msg-title', 'alert-danger');
            $this->session->set_flashdata('msg', 'Terjadi Kesalahan');
            redirect('mahasiswa/tesis/mkpt/bimbingan/'.$id_tesis);
