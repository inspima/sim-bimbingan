	...
	<div class="col-sm-6">
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div class="form-group">
					<label>Berkas Skripsi</label>
					<p>
						<a href="<?php echo base_url() ?>assets/upload/skripsi/<?php echo $skripsi->berkas_skripsi ?>" target="_blank"><img src="<?php echo base_url() ?>assets/img/pdf.png" width="50px" height="auto"></a>
					</p>
				</div>
	...
	<div class="col-sm-6">
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<?php echo form_open('dosen/sarjana/kadep/skripsi/ujian_ulang_save'); ?>
				<?php
					$ujian = $this->transaksi_skripsi->read_ujian_aktif($skripsi->id_skripsi, UJIAN_SKRIPSI_UJIAN);
		...
		<div class="box box-primary">
			...
			<div class="box-body table-responsive">
				<?php
					$is_update = 0;
					if (!$is_update) {
						$ujian = $this->transaksi_skripsi->read_ujian_selesai($skripsi->id_skripsi, UJIAN_SKRIPSI_UJIAN);
					}
					if ($ujian) {
						?>
						<?php echo form_open('dosen/sarjana/kadep/skripsi/penguji_save'); ?>
						<div class="form-group">
							<label>Penguji</label>
							<?php
								if ($is_update) {
								...
								<?php
									...
									foreach ($penguji as $listpenguji) {
										...
										<tr>
											...
											<td>
												<?php
													if ($listpenguji['status_tim'] == '1') {
														?>
														<?php echo form_open('dosen/sarjana/kadep/skripsi/penguji_update_statustim') ?>
														<?php echo formtext('hidden', 'hand', 'center19', 'required') ?>
														<?php echo formtext('hidden', 'id_skripsi', $skripsi->id_skripsi, 'required') ?>
														<?php echo formtext('hidden', 'id_ujian', $id_ujian, 'required') ?>
											...
											<td>
												<?php echo form_open('dosen/sarjana/kadep/skripsi/penguji_delete') ?>
												<?php echo formtext('hidden', 'hand', 'center19', 'required') ?>
												<?php echo formtext('hidden', 'id_skripsi', $skripsi->id_skripsi, 'required') ?>
												<?php echo formtext('hidden', 'id_ujian', $id_ujian, 'required') ?>
												<?php echo formtext('hidden', 'id_penguji', $listpenguji['id_penguji'], 'required') ?>
