			...
			<div class="box-body">
				<?php
					if (!empty($biodata->berkas_verifikasi)) {
						...
						<div class="callout callout-info">Silahkan simpan ulang data anda, jika mendapatkan notifikasi verifikasi ulang</div>
						<?php
					}
				?>
				<table class="table table-bordered">
							...
							<select name="prodi" class="form-control">
								...
								<?php
									foreach ($prodis as $prodi):
										?>
										<option value="<?= $prodi['id_prodi'] ?>" <?= $prodi['id_prodi'] == $biodata->id_prodi ? 'selected' : '' ?>><?= $prodi['jenjang'] ?> <?= $prodi['nm_prodi'] ?></option>
									<?php
									endforeach;
								?>
							</select>
					...
					<tr>
						...
						<td style="width: 5%">:</td>
						<td>
							<input name="no_hp" type="text" class="form-control" value="<?php echo str_replace('+62','0',$biodata->no_hp) ?>"/>
						</td>
					</tr>
					<tr>
